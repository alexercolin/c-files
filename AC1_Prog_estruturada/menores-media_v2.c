...
void findValuesLowerThanAverage(int *value, int size,  int average);
...
int main() {
    ...
    findValuesLowerThanAverage(input_values, size, average);
...
void findValuesLowerThanAverage(int *value, int size, int average) {
    int *lower_values = NULL;
    int lower_size = 0;
    
    for(int i = 0; i < size; i++, value++){
        if(*value < average){
            allocate(&lower_values, lower_size+1);
            *(lower_values+lower_size) = *value;
            lower_size++;
        }
    }

    showValues(lower_values, lower_size);
}
